...
const appConfig = {
  ...
  pollIntervalMs: Math.max(Number(process.env.POLLING_INTERVAL || process.env.JUMPSELLER_POLL_INTERVAL_MS || 60000), 5000),
  sectoriceApiUrl: (process.env.URL_BACKEND || process.env.SECTORICE_API_URL || 'https://sectorice.cl').replace(/\/$/, ''),
  internalAdapterToken: process.env.INTERNAL_ADAPTER_TOKEN || process.env.ECOMMERCE_ADAPTER_INTERNAL_TOKEN || '',
  integrationIdentifier: 'sectorice-jumpseller-app',
...
const server = app.listen(appConfig.port, () => {
  ...
  console.log(`[sectorice-jumpseller-app] Backend ${appConfig.sectoriceApiUrl}`);
